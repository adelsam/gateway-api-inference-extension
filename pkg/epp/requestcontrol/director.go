...
func (d *Director) HandleResponseBody(ctx context.Context, reqCtx *handlers.RequestContext, endOfStream bool) *handlers.RequestContext {
	...
	if response.DynamicMetadata != nil {
		logger.V(logutil.VERBOSE).Info("Before DynamicMetadata assignment", "reqCtx.Response.DynamicMetadata (old)", reqCtx.Response.DynamicMetadata, "response.DynamicMetadata (new)", response.DynamicMetadata)
		reqCtx.Response.DynamicMetadata = response.DynamicMetadata
		logger.V(logutil.VERBOSE).Info("After DynamicMetadata assignment", "reqCtx.Response.DynamicMetadata", reqCtx.Response.DynamicMetadata)
	} else {
		logger.V(logutil.VERBOSE).Info("Plugin returned nil DynamicMetadata, keeping existing", "current", reqCtx.Response.DynamicMetadata)
	}
	logger.V(logutil.VERBOSE).Info("Exiting HandleResponseBodyChunk", "reqCtx.Response.DynamicMetadata", reqCtx.Response.DynamicMetadata)
	return reqCtx
}
